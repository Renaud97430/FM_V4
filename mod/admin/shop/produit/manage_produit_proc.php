<?php
    ...
    if(isset($_POST) && !empty($_POST)){
        ...
        $h = array();
        $h['prixHT'] = $_POST['form_prixHT'];
        $h['poids'] = $_POST['form_poids'];
        $h['fk_tva'] = $_POST['form_fk_tva'];
        $h['fk_promotion'] = $_POST['form_fk_promotion'];
    ...
    if($id_produit > 0){
        $html .= '       <h1>Modification Produit<span>Modifier un produit...</span></h1>';
    }else{
        $html .= '       <h1>Ajout Produit<span>Ajouter un Produit...</span></h1>';
    }
    ...
    foreach($datas_langue as $data_langue) {
        ...
        if($first) {
            ...
            $html .= '              <div class="section">Informations produit</div>';
            $html.= '               <ul>';
    ...
    $html.= '           <div class="section"><span>'.++$i.'</span>Images du produit</div>';
    $html.= '           <div class="inner-wrap-l">';
    ...
    // Gestion Tarif / TVA / Poids / Promotion / isActif
    $html.= '           <div class="section"><span>'.++$i.'</span>Tarif, TVA, Poids et Promotion</div>';
    $html.= '           <div class="inner-wrap-l">';
    $html.= '               <label> Tarif HT<input type="number" step=".01" name="form_prixHT" value="'.$data_produit['prixHT'].'" /></label>';

    $sql = "SELECT * FROM t_tva ORDER BY nom_tva ASC";
    $datas_tva = $bdd->getData($sql);
    $html.= '               <label> TVA ';
    $html.= '                   <select name="form_fk_tva">';
    $html.= '                       <option value="0">-- Selection TVA --</option>';
    if($datas_tva) {
        foreach($datas_tva as $data_tva) {
            $html.= '<option value="'.$data_tva['id'].'" '.(($data_produit['fk_tva'] == $data_tva['id'])?' selected="selected" ':'').'>'.$data_tva['nom_tva'].'</option>';
        }
    }
    $html.= '                   </select>';
    $html.= '               </label>';
    $html.= '               <label> Poids (en Kg)<input type="number" step=".01" name="form_poids" value="'.$data_produit['poids'].'" /></label>';

    $html.= '           </div>';
    $html.= '           <div class="inner-wrap-r">';

    $sql = "SELECT p.id, pt.nom";
    ...
    $html.= '                   <select name="form_fk_promotion">';
    $html.= '                       <option value="0">-- Selection Promotion --</option>';
    if($datas_tva) {
        foreach($datas_promotion as $data_promotion) {
            $html.= '<option value="'.$data_promotion['id'].'" '.(($data_produit['fk_promotion'] == $data_promotion['id'])?' selected="selected" ':'').'>'.$data_promotion['nom'].'</option>';
        }
    }
    $html.= '                   </select>';
    $html.= '               </label>';
    if(isset($data_produit['date_debut_promotion']) && !empty($data_produit['date_debut_promotion']))
        $html.= '               <label> Date Début Promotion<input type="date" name="form_date_debut_promotion" value="'.date('Y-m-d',$data_produit['date_debut_promotion']).'" /></label>';
    else
        $html.= '               <label> Date Début Promotion<input type="date" name="form_date_debut_promotion" value="" /></label>';
    if(isset($data_produit['date_fin_promotion']) && !empty($data_produit['date_fin_promotion']))
        $html.= '               <label> Date Fin Promotion<input type="date" name="form_date_fin_promotion" value="'.date('Y-m-d',$data_produit['date_fin_promotion']).'" /></label>';
    else
        $html.= '               <label> Date Fin Promotion<input type="date" name="form_date_fin_promotion" value="" /></label>';
    $html.= '           </div>';
    $html.= '           <div style="clear:both;"></div>';
    ...
    // Gestion du stock (Version simple et statique...)
    $html.= '           <div class="section"><span>'.++$i.'</span>Gestion du Stock</div>';
    $html.= '           <div class="inner-wrap">';
    ...
    if($datas_stock) {
        foreach ($datas_stock as $data_stock) {
            ...
            $html.= '               <label> '.$data_stock['nom'].'<input type="text" name="form_stock_'.$data_stock['id'].'" value="'.$qte_stock.'" /></label>';
        }
    }
    $html.= '           </div>';
    $html.= '           <div style="clear:both;"></div>';
