<?php
    ...
    if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
        ...
        // Traduction
        $bdd->squery("DELETE FROM t_produit_trad WHERE fk_produit=".$id_produit);
        ...
        $bdd->squery("DELETE FROM t_produit_image WHERE fk_produit=".$id_produit);

        // Stock
        $bdd->squery("DELETE FROM t_produit_stock WHERE fk_produit=".$id_produit);
    ...
    $html .= '                  <td class="tab_td">Quantité</td>';
    $html .= '                  <td class="tab_td">Prix HT</td>';
    $html .= '                  <td class="tab_td">Code TVA</td>';
    $html .= '                  <td class="tab_td">Prix TTC</td>';
    ...
    if($datas_produit) {
        foreach ($datas_produit as $data) {
            ...
            $html .= '            <td class="tab_td">' . $data['qte'] . '</td>';
            $html .= '            <td class="tab_td">' . $data['prixHT'] . '</td>';
            $html .= '            <td class="tab_td">' . $data['tva'] . '</td>';
            $html .= '            <td class="tab_td">' . number_format($data['prixTTC'], 2) . '</td>';
            if ($data['isActif'])
