<?php
    ...
    if($data_produit['reduction']>0) {
        $html .= '            <div class="zone_information_product_price">';
        $html .= '                '.number_format($data_produit['prixTTC'] - ($data_produit['prixTTC'] / 100 * $data_produit['reduction']),2).' €';
        $html .= '                <span>' . number_format($data_produit['prixTTC'], 2) . ' € </span><span class="zone_information_product_price_promo"> -'.ceil($data_produit['reduction']).' %</span>';
        $html .= '            </div>';
    } else {
        $html .= '            <div class="zone_information_product_price">';
    ...
    if($data_produit['qte']>0) {
        $html .= '            <div class="zone_information_product_stock_ok">';
        $html .= '                Il reste ' . $data_produit['qte'] . ' unités en stock';
        $html .= '            </div>';
    } else {
        $html .= '            <div class="zone_information_product_stock_ko">';
