...
if (isset($_POST) && !empty($_POST)) {
   ...
   //Gestion de la table t_commande
   $h = array();
   ...
   //Gestion de la table t_commande_produit
   ...
   header('Location: index.php?page=fo_commandeAR');
   // echo "<pre>";
   // var_dump($_SESSION);
   // echo "</pre>";
   // exit();
...
if (!empty($_SESSION[SESSION_NAME]['panier'])) {
   ...
   foreach ($_SESSION[SESSION_NAME]['panier'] as $data_produit_cart) {
      ...
      $html .= '            <td>';
      $html .= '                <a onclick="if(window.confirm(\'Etes vous sur ?\')) return true; else return false;" href="index.php?page=panier&del_id_produit=' . $data_produit_cart['id_produit'] . '" style="text-decoration:none;">';
      ...
      if ($data_produit['fk_promotion']) {
         ...
         //$html .= '                <img src="images/interface/icone_promo.png" style="vertical-align: middle;" />';
   ...
   // Validation Panier
   $html .= '<form method="POST" action="index.php?page=panier" enctype="multipart/form-data">';
